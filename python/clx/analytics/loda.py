...
class Loda:
    ...
    def score(self, input_data):
        ...
        for i in range(self._n_random_cuts):
            ...
            inds = cp.searchsorted(
                self._limits[i, : self._n_bins - 1], projected_data, side="left"
            )
    ...
    def explain(self, anomaly, scaled=True):
        ...
        if scaled:
            ...
            normalized_score = (
                ranked_feature_importance - cp.min(ranked_feature_importance)
    ...
    def _t_test(self, with_sample, without_sample):
        ...
        compute one-tailed two-sample t-test with a test statistics according to
            ...
            \frac{\bar{s^2_j}}{\norm{\bar{I_j}}}}}
        """
        return (cp.mean(with_sample) - cp.mean(without_sample)) / cp.sqrt(
            cp.var(with_sample) ** 2 / len(with_sample)
