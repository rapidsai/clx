...
/*
  ...
  Params
  ---------
  num_48bit: A multplicand that is at most 48 bits.
...
*/
__device__ __forceinline__ uint64_t sdbm_hash(uint32_t* sequence_start, uint32_t length, uint64_t start_value = 0) {
...
/*
  The hash function used for accesses to the table. This is a universal hash function with parameters
  chosen to achieve perfect hashing.

  returns h(k, a, b, table_size) = ((a*k + b % PRIME) % table_size) where PRIME is globally defined as
          281474976710677
*/
...
*/
__device__ __forceinline__ int retrieve(uint64_t key, uint32_t outer_table_a, uint32_t outer_table_b, uint16_t num_bins,
                                        uint64_t* hash_table, uint64_t* bin_coefficients, uint16_t* bin_offsets) {
...
/*
  ...
  This describes a two level hashing sceheme based on the FKS perfect hashing algorithm.
  ...
  first_tok_id: A reference to the variable where the token id for the starting of the first sentence is stored
...
void transfer_hash_info_to_device(const std::string hash_data_file, rmm::device_vector<uint64_t>& device_hash_table, 
                                  rmm::device_vector<uint64_t>& device_bin_coefficients, uint16_t** device_bin_offsets,
