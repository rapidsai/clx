...
def train(dd, partitioned_dfs, dataset_len, test_partitioned_dfs, test_dataset_len, epoch, output_dir):
    ...
    for i in range(1, epoch + 1):
        ...
        now = datetime.now()
        output_filepath = output_dir + "/" + "rnn_classifier_{}.pth".format(
    ...
    log.info("Model with highest accuracy (%s) is stored to location %s" % (max_accuracy, prev_model_file_path))       
...
def main():
    ...
    train(dd, partitioned_dfs, dataset_len, test_partitioned_dfs, test_dataset_len, epoch, output_dir)
